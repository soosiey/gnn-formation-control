...
class RobotDataset(Dataset):
    ...
    def __init__(self, data1, data2, data3, data4, data5, inW = 100, inH = 100, transform = None):
        self.obs = data1
        self.inW = inW
        self.inH = inH
        c1 = np.zeros((self.obs.shape[0]//3,3,self.inW,self.inH))
        jump = c1.shape[0]
        ...
        for i in tqdm(range(len(c1))):
            ...
            c1[i,0] = self.obs[i].reshape((self.inW, self.inH))
        ...
        self.gt = data2
        c2 = np.zeros((self.obs.shape[0],3,2))
