...
parser.add_argument('--model_name', dest='model_name', default='model_train_episode-110_robot-7.pth',type=str,help='Name of model')
...
parser.add_argument('--train_episode', dest='train_episode', default=210,type=int,help='Episode for training')
...
def Train(args):
    ...
    for i in range(episode,numRun):
        ...
        sc = generate_scene(dt=args.sim_dt, num_run=i, robot_num=args.robot_num, if_train=args.if_train,
        ...
        if sc0 is not None:
            ...
            if not dataList:
                ...
                for j in range(2,len(sc.robots)):
        ...
        if i % args.save_iteration == 0 and args.if_train:
            ...
            fcl.save(last_model)
    saved_model_name = "model_train_circle" + "_episode-" + str(numRun) + "_robot-" + str(args.robot_num) + ".pth"
    saved_model = os.path.join(args.model_path, saved_model_name)
    fcl.save(saved_model)
    time_file = os.path.join(args.model_path, "time.txt")
    with open(time_file, 'a') as time_log:
